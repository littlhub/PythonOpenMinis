...
Kotlin parity is the point of this module: the *four* strategies, their exact
thresholds (10 / 20 / 30), the priority order and the constructor invariants
are copied verbatim from the Kotlin original. Anything we invented on top of it
(retrieval-family runaway, effect-tool repeat) lives in ``repeat_guard.py`` as
an independent runtime-level strategy — see that module's docstring.

Strategy priority (highest first), identical to Kotlin:
...
__all__ = [
    ...
    "ToolLoopDetector", "args_hash_for", "sha256",
]
...
class ToolLoopConfig:
    """Kotlin ``data class ToolLoopConfig`` — same defaults, same invariants.

    Threshold contract (enforced at construction, like Kotlin's ``init{}``):
    ``warningThreshold < criticalThreshold < globalCircuitBreakerThreshold``
    and ``historySize >= globalCircuitBreakerThreshold``.
    """
    ...
    def __post_init__(self) -> None:
        if self.warning_threshold <= 0:
            raise ValueError("warningThreshold must be positive")
        if self.warning_threshold >= self.critical_threshold:
            raise ValueError(
                f"warningThreshold ({self.warning_threshold}) must be "
                f"< criticalThreshold ({self.critical_threshold})"
            )
        if self.critical_threshold >= self.global_circuit_breaker_threshold:
            raise ValueError(
                f"criticalThreshold ({self.critical_threshold}) must be "
                f"< globalCircuitBreakerThreshold "
                f"({self.global_circuit_breaker_threshold})"
            )
        if self.history_size < self.global_circuit_breaker_threshold:
            raise ValueError(
                f"historySize ({self.history_size}) must be >= "
                f"globalCircuitBreakerThreshold "
                f"({self.global_circuit_breaker_threshold})"
            )
...
class ToolLoopDetector:
    """Sliding-window repeat / no-progress detector (Kotlin parity).
    ...
    One detector instance per session (see ``chat_service.session_guards``).
    Not thread-safe — serialize calls through the agent loop's dispatch.
    """
    ...
    def record(
        ...
        """
        args_hash = args_hash_for(tool_name, params)
        result_hash = result_hash_for(result, error_message)
        unknown_tool = extract_unknown_tool_name(error_message)
...
# ─── hashing / parsing primitives (shared with repeat_guard) ────────────────
...
def stable_json(value: Any) -> str:
    """Stable JSON: keys sorted alphabetically at every nesting level so equal
    maps (regardless of insertion order) hash identically."""
...
def args_hash_for(tool_name: str, params: dict[str, Any]) -> str:
    """Hash a tool call's identity, ignoring UI-only fields."""
    if any(k in params for k in ARGS_HASH_IGNORED_KEYS):
        filtered = {k: v for k, v in params.items()
                    if k not in ARGS_HASH_IGNORED_KEYS}
    else:
        filtered = params
    return sha256(f"{tool_name}:{stable_json(filtered)}")


def result_hash_for(result: Optional[str], error_message: Optional[str]) -> str:
...
def extract_unknown_tool_name(error_message: Optional[str]) -> Optional[str]:
    """Two patterns cover the wording variations seen across providers:
    ``unknown tool: foobar`` and ``tool 'foobar' not found``."""
